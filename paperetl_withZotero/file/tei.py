...
from .Zotero_extractor import parse_zotero_metadata
...
class TEI:
    ...
    @staticmethod
    def parse(stream, source, zotero_metadata=None):
        """
        ...
        if zotero_metadata:
            (
                ...
                affiliation,
                reference,
            ) = ic(parse_zotero_metadata(metadata_dict=zotero_metadata))
        else:
            # Extract article metadata if no zotero metadata is present
            (
                published,
                publication,
                authors,
